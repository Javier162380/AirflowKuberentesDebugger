...
def test_generate_pod_template_integration(mocker, AirflowtoYamlClient):
    ...
    expected_calls = [
        {'apiVersion': 'v1', 'kind': 'Pod',
         'metadata': {'name': 'pod-ex-minimum', 'labels': {},
                      'annotations': {},
                      'namespace': 'default'},
         'spec':
         {
        ...
        {'apiVersion': 'v1', 'kind': 'Pod',
         'metadata': {'name': 'ex-kube-templates', 'labels': {},
                      'annotations': {},
                      'namespace': 'default'},
         'spec':
         {
             ...
             'serviceAccountName': 'default', 'affinity': {}}},
        {'apiVersion': 'v1', 'kind': 'Pod',
            'metadata': {'name': 'ex-kube-secrets', 'labels': {},
                         'annotations': {},
                         'namespace': 'default'},
            'spec':
            {
                ...
                'volumes': [],
                'serviceAccountName': 'default', 'affinity': {}}},
        {'apiVersion': 'v1', 'kind': 'Pod', 'metadata': {'name': 'ex-pod-affinity', 'labels': {}, 'annotations': {}, 'namespace': 'default'},
         'spec':
         {
            'containers':
            [{'name': 'base', 'image': 'perl', 'command': ['perl'], 'imagePullPolicy': 'IfNotPresent',
              'args': ['-Mbignum=bpi', '-wle', 'print bpi(2000)'], 'ports': [
            ]}],
            'restartPolicy': 'Never', 'nodeSelector': {},
            'volumes': [], 'serviceAccountName': 'default', 'affinity': {'nodeAffinity': {'requiredDuringSchedulingIgnoredDuringExecution': {'nodeSelectorTerms': [{'matchExpressions': [{'key': 'cloud.google.com/gke-nodepool', 'operator': 'In', 'values': ['pool-0', 'pool-1']}]}]}}}}},
        {'apiVersion': 'v1', 'kind': 'Pod',
         'metadata':
         {'name': 'pi', 'labels': {'pod-label': 'label-name'},
